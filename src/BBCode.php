...
namespace nguyenanhung\Libraries\BBCode;

/**
 * Class BBCode
 *
 * @package   nguyenanhung\Libraries\BBCode
...
class BBCode
{
    ...
    public function __construct()
    {
        ...
        $this->bbcode_table["/\[b\](.*?)\[\/b\]/is"] = static function ($match) {
            return "<strong>$match[1]</strong>";
        };


        // Replace [i]...[/i] with <em>...</em>
        $this->bbcode_table["/\[i\](.*?)\[\/i\]/is"] = static function ($match) {
            return "<em>$match[1]</em>";
        };


        // Replace [code]...[/code] with <pre><code>...</code></pre>
        $this->bbcode_table["/\[code\](.*?)\[\/code\]/is"] = static function ($match) {
            return "<pre><code>$match[1]</code></pre>";
        };


        // Replace [quote]...[/quote] with <blockquote><p>...</p></blockquote>
        $this->bbcode_table["/\[quote\](.*?)\[\/quote\]/is"] = static function ($match) {
        ...
        $this->bbcode_table["/\[url\](.*?)\[\/url\]/is"] = static function ($match) {
        ...
        $this->bbcode_table["/\[img\](.*?)\[\/img\]/is"] = static function ($match) {
            return "<img alt='' src=\"$match[1]\"/>";
        };


        // Replace [list]...[/list] with <ul><li>...</li></ul>
        $this->bbcode_table["/\[list\](.*?)\[\/list\]/is"] = static function ($match) {
            $match[1] = preg_replace_callback("/\[\*\]([^\[\*\]]*)/is", static function ($submatch) {
        ...
        $this->bbcode_table["/\[list=(1|a)\](.*?)\[\/list\]/is"] = static function ($match) {
            if ($match[1] === '1') {
                $list_type = '<ol>';
            } elseif ($match[1] === 'a') {
            ...
            $match[2] = preg_replace_callback("/\[\*\]([^\[\*\]]*)/is", static function ($submatch) {
                return "<li>" . preg_replace("/[\n\r?]$/", "", $submatch[1]) . "</li>";
            },                                $match[2]);
        ...
        $this->bbcode_table["/\[youtube\](?:http?:\/\/)?(?:www\.)?youtu(?:\.be\/|be\.com\/watch\?v=)([A-Z0-9\-_]+)(?:&(.*?))?\[\/youtube\]/i"] = static function ($match) {
            return "<iframe class=\"youtube-player\" type=\"text/html\" width=\"640\" height=\"385\" src=\"https://www.youtube.com/embed/$match[1]\" frameborder=\"0\"></iframe>";
        };
    }
